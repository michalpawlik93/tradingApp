...
public class GetStooqCombinedQuotesCommandHandlerTests
{
    ...
    public async Task Handle_GetQuotesFailed_ResponseReturned(GetStooqCombinedQuotesCommand command)
    {
        ...
        StooqProvider.Setup(_ => _.GetQuotes(command.HistoryType)).ReturnsAsync(Result.Fail<IEnumerable<Quote>>(errrorMessage));
        //Act
    ...
    {
        //Arrange
        StooqProvider.Setup(_ => _.GetQuotes(command.HistoryType)).ReturnsAsync(Result.Ok(quotes));
